...
class RNNBase(torch.nn.Module):
    def __init__(self, input_size, output_size, hidden_size_list, activation, layer_type, logger=None,
                 aux_dim=0, new_state_dict=True):
        super().__init__()
        ...
        elif self.layer_type[-1].startswith('efc'):
            self.layer_list.append(fc_decorate(EnsembleLinear(lst_nh, output_size, int(self.layer_type[ind].split('-')[-1]))))
        else:
            self.rnn_num += 1
    ...
    def set_all_layer_spectral_norm(self):
        for i in range(len(self.layer_list)):
            self.layer_list[i] = torch.nn.utils.spectral_norm(self.layer_list[i])
