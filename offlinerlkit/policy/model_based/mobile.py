...
class MOBILEPolicy(BasePolicy):
    ...
    def rollout(
        ...
        for step in range(rollout_length):
            ...
            next_observations, rewards, terminals, info = self.dynamics.step(observations, actions, clipping_value=self._state_clip, current_step=step+1)
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        with torch.no_grad():
            # accelerate
            penalty = self.compute_lcb(obss[len(real_batch["rewards"]):], actions[len(real_batch["rewards"]):])
            penalty = torch.cat((torch.zeros((len(real_batch["rewards"]),) + penalty.shape[1:], device=penalty.device, dtype=penalty.dtype),
                                 penalty), dim=0)
            # penalty[:len(real_batch["rewards"])] = 0.0

            next_actions, next_log_probs = self.actforward(next_obss)
            # accelerate
            if len(self.critics_old) == 2:
                next_q = torch.min(self.critics_old[0](next_obss, next_actions),
                                   self.critics_old[1](next_obss, next_actions)).reshape((-1, 1))
            else:
        ...
        if len(self.critics) == 2:
            actor_loss = -torch.min(self.critics_old[0](obss, a),
                                   self.critics_old[1](obss, a)).mean() + self._alpha * log_probs.mean()
        else:
