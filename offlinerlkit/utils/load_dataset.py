...
def qlearning_dataset(env, dataset=None, terminate_on_end=False, only_load_initial=False, **kwargs):
    """
    ...
    for i in range(N-1):
        ...
        if episode_step == 0 and only_load_initial:
            init_obs_.append(obs)
            init_next_obs_.append(new_obs)
            init_action_.append(action)
            init_reward_.append(reward)
            init_done_.append(done_bool)
    ...
    if only_load_initial:
        obs_ = init_obs_
        next_obs_ = init_next_obs_
        action_ = init_action_
        reward_ = init_reward_
        done_ = init_done_
    return {
