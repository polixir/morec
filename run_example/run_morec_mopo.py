...
class Parameter(ParameterTemplate):
    ...
    def parser_init(self):
        ...
        parser.add_argument("--reward_use_type", type=str, default=self.reward_use_type, choices=['select_elite', 'softmax_elite'])
...
def train(args, parameters:Parameter):
    ...
    critic1_eval = Critic(critic1_backbone_eval, 'cpu')
    critic2_eval = Critic(critic2_backbone_eval, 'cpu')
    actor_eval_optim = torch.optim.Adam(actor_eval.parameters(), lr=args.actor_lr)
    critic1_eval_optim = torch.optim.Adam(critic1_eval.parameters(), lr=args.critic_lr)
    critic2_eval_optim = torch.optim.Adam(critic2_eval.parameters(), lr=args.critic_lr)
    policy_eval = SACPolicy(
        actor_eval,
        critic1_eval,
        critic2_eval,
        actor_eval_optim,
    ...
    #     dynamics.train(real_buffer.sample_all(), logger, slogger_model, max_epochs_since_update=5)
    if not load_dynamics_model:
